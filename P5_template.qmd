...
df.head()  # just to check what the data looks like
...
seen_col = 'Which of the following Star Wars films have you seen? Please select all that apply.'
gender_col = 'Gender'

# Create a column that is True if the person saw at least one film
df['seen_any_film'] = df[seen_col].notna()

# Filter only male respondents
male_df = df[df[gender_col] == 'Male']

# Calculate percentage of males who have seen at least one film
percent_males_seen = male_df['seen_any_film'].mean() * 100

# Print result
print(f"Percentage of males who have seen at least one film: {percent_males_seen:.4f}%")
...
# Check exact name of the column (copy-paste it from your list)
col_name = 'Which of the following Star Wars films have you seen? Please select all that apply.'

# Create a new column: True if they’ve selected any film (i.e., the cell is not null)
df['seen_any_film'] = df[col_name].notna()

# Calculate the percentage
percentage_seen = df['seen_any_film'].mean() * 100

# Print the result to four decimal places
print(f"Percentage of people who have seen at least one film: {percentage_seen:.4f}%")
